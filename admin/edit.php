    ...
    if ($_FILES['image']['error'] > 0) {
        ...
        if (strlen(strstr($file_type, 'image')) > 0) {
            if ((round($file_size / 1024, 0)) <= 10240) {
                ...
                move_uploaded_file($_FILES['image']['tmp_name'], 'img/' . $result . $file_name);
...
if (isset($_POST['name']) && count($err) == 0) {
    ...
    include_once('db.php');
    $sql = "UPDATE sanpham SET name_sp ='$name_product',gia_sp='$cost_product', img_sp='$image', mota_sp='$mota' WHERE id_sp = '$id'";
    $conn->exec($sql);
